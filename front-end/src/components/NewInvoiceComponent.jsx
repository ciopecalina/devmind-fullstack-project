...
import { getStockProducts, getClientsNames } from "../api/InvoicingAppApi";
...
const NewInvoiceComponent = ({ open, onClose, onSave }) => {
    ...
    const handleAddProduct = () => {
        const { name, uom, quantity, unitPrice } = newProduct;
        if (!name || !quantity || !unitPrice || quantity <= 0 || unitPrice <= 0) {
            return;
        }

        const totalNoVat = (quantity * unitPrice).toFixed(2);
        const vat = (totalNoVat * VAT_RATE).toFixed(2);
        const totalWithVat = (parseFloat(totalNoVat) + parseFloat(vat)).toFixed(2);

        setProducts([...products, { name, uom, quantity, unitPrice, totalNoVat, vat, totalWithVat }]);
        setNewProduct({ name: "", uom: "", quantity: "", unitPrice: "" });
    };
                    ...
                    <Box display="flex" gap={2}>
                        ...
                        <Box width="50%" display="flex" flexDirection="column" gap={1}>
                            ...
                            <Autocomplete
                                ...
                                renderInput={(params) => <TextField {...params} label="Client Name" size="small" fullWidth />}
                            />
                        </Box>
                    </Box>
                </Box>
                    ...
                    <Box display="flex" gap={2} mb={1}>
                        <Box flex={2}>
                            <Autocomplete
                                ...
                                renderInput={(params) => <TextField {...params} label="Product Name" size="small" fullWidth />}
                            />
                        </Box>
                        <Box width="80px">
                            <TextField size="small" label="UOM" value={newProduct.uom}
                                       onChange={(e) => setNewProduct({ ...newProduct, uom: e.target.value })} fullWidth/>
                        </Box>
                        <Box width="100px">
                            <TextField size="small" label="Quantity" type="number" value={newProduct.quantity}
                                       onChange={(e) => setNewProduct({ ...newProduct, quantity: +e.target.value })} fullWidth/>
                        </Box>
                        <Box width="120px">
                            <TextField size="small" label="Unit Price" type="number" value={newProduct.unitPrice}
                                       onChange={(e) => setNewProduct({ ...newProduct, unitPrice: +e.target.value })} fullWidth/>
                        </Box>
                    </Box>

                    <Box display="flex" justifyContent="flex-end" gap={2} mt={2}>
                        <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={handleAddProduct} size="small">
                            Add Product
                        </Button>
                        <Button variant="contained" color="error" startIcon={<DeleteIcon />} onClick={handleDeleteProduct} size="small">
                            Delete Product
                        </Button>
                    </Box>
                </Box>
